    ...
    const handleSubmit = async () => {
        ...
        if (!err) {
            ...
            if (parseFloat(calculatedBMI) < 18.5) {
                status = 'Underweight';
                setAdvice('Bạn đang thiếu cân, bạn nên ăn nhiều bữa nhỏ trong ngày thay vì ba bữa chính.');
                setHelpfulAdvice(
                ...
                setAdvice('Hãy duy trì lối sống lành mạnh để giữ gìn cân nặng và sức khỏe tốt.')
                setHelpfulAdvice(
                ...
                status = 'Overweight';
                setAdvice('Bạn nên giảm cân để giảm nguy cơ mắc các bệnh mãn tính như bệnh tim, đột quỵ, tiểu đường loại 2 và một số loại ung thư.')
                setHelpfulAdvice(
                ...
                status = 'Obesity';
                setAdvice('Bạn nên giảm cân càng sớm càng tốt để cải thiện sức khỏe và giảm nguy cơ mắc các bệnh nghiêm trọng.')
                setHelpfulAdvice(
