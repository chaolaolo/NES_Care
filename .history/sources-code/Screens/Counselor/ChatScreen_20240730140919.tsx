...
const ChatScreen = ({ route }) => {
    ...
    const sendMessage = async () => {
        if (message.trim()) {
            const newMessage = {
                ...
                chatId: chatId,
                sender: {
                    uid: uid, // ID người gửi
                    fullName: fullName,
                    avatar: avatar,
                },
                receiver: {
                    uid: route.params.opponentUid, // ID người nhận
                    fullName: route.params.opponentFullName,
                    avatar: route.params.opponentAvatar,
                },
            };
    ...
    const renderItem = ({ item }) => {
        const isCurrentUser = item.sender.uid === uid;
        const isSelected = item.id === selectedMessageId;

        return (
            <Pressable onPress={() => handlePressMessage(item.id)}>
                <View style={[styles.messageRow, isCurrentUser ? styles.messageRowRight : styles.messageRowLeft]}>
                    {!isCurrentUser && <Image source={{ uri: item.sender.avatar }} style={[styles.avatar, { width: 30, height: 30 }]} />}
                    <View style={isCurrentUser ? styles.messageContainerRight : styles.messageContainerLeft}>
                        <Text style={styles.messageText}>{item.text}</Text>
                        {isSelected && <Text style={styles.messageTime}>{item.createdAt.toLocaleString()}</Text>}
                    </View>
    ...
    return (
        <KeyboardAvoidingView
            ...
            <View style={{ paddingTop: 100, marginBottom: 100 }}>
                <FlatList
                    ref={flatListRef}
                    data={messages}
                    renderItem={renderItem}
