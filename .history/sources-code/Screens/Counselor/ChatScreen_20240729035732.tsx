...
const ChatScreen = ({ route }) => {
    ...
    const currentUserId = "current_user_uid"; // Thay thế bằng ID người dùng hiện tại
    ...
    const sendMessage = async () => {
        if (message.trim()) {
            const newMessage = {
                ...
                chatId: chatId,
                user: {
                    uid: currentUserId, // Thay thế bằng ID người dùng hiện tại
                    fullName,
                },
            };
    ...
    const renderItem = ({ item }) => {
        const isCurrentUser = item.user.uid === currentUserId;
        const isSelected = item.id === selectedMessageId;
