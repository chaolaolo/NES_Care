...
const ChatScreen = ({ route }) => {
        ...
        const unsubscribe = firestore()
            ...
            .onSnapshot(querySnapshot => {
                if (querySnapshot) {
                    ...
                    setMessages(messages);
                    flatListRef.current?.scrollToEnd({ animated: true }); // Cuộn danh sách khi dữ liệu thay đổi
                }
            });
    ...
    const sendMessage = async () => {
        if (message.trim()) {
            ...
            try {
                ...
                setMessages(prevMessages => [...prevMessages, newMessage]);
                setMessage('');
                flatListRef.current?.scrollToEnd({ animated: true }); // Cuộn danh sách khi gửi tin nhắn mới
            } catch (error) {
                console.error("Error sending message: ", error);
            }
        }
    ...
    return (
        <KeyboardAvoidingView
            ...
            >
                <FlatList
                    ref={flatListRef} // Gán tham chiếu cho FlatList
                    data={messages}
                    renderItem={renderItem}
                    keyExtractor={(item, index) => index.toString()}
                    onContentSizeChange={() => flatListRef.current?.scrollToEnd({ animated: true })} // Cuộn đến cuối khi kích thước nội dung thay đổi
                />
            </View>
