...
const Sleep = () => {
  ...
  const saveSleepSettings = async () => {
    if (!timeStart || !timeStop) {
      Alert.alert("Lỗi", "Vui lòng nhập cả giờ đi ngủ và giờ thức dậy.");
      return;
    }

    const start = convertTo24HourFormat(timeStart);
    const stop = convertTo24HourFormat(timeStop);

    let sleepDuration = (stop - start) / (1000 * 60 * 60);
    ...
    return (
      ...
      <View style={{
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          ...
          <Text style={{ fontSize: 18, flex: 1 }}>Báo thức: {item.checkAlarm ? 'Có' : 'Không'}</Text>
        </View>
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
