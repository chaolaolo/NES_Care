...
const Music = () => {
    ...
    const handlePlay = async (item) => {
        try {
            ...
            if (currentTrack === null || currentTrack !== item.key) {
                if (!tracks.some(track => track.id === item.key)) {
                    await TrackPlayer.reset();
                    await TrackPlayer.add({
                        id: item.key,
                        url: item.link,
            ...
            await updateTrackInfo(); // Ensure track info is updated after play/pause
        } catch (error) {
            console.error("Error handling track play: ", error);
        }
    };
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            <View style={styles.playingContainer}>
                <Image style={styles.imgPlayingCover} source={url_cover ? { uri: url_cover } : require('../../image/ic_sound.png')} />
                <View style={{ flex: 1, marginHorizontal: 16 }}>
                    <Text style={styles.musicName}>{musicName ? musicName : "Unknow Name"}</Text>
                    <Text style={styles.musicArtist}>{musicArtist ? musicArtist : "Unknow Artist"}</Text>
