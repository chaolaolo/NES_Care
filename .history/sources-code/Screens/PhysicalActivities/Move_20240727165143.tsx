...
const Move = () => {
  ...
  const startCounting = () => {
    ...
    setIsCounting(!isCounting); // Toggle isCounting on each press
    if (isCounting) {
      setStartTime(Date.now());
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      /> */}
      <ButtonComponent 
  title={isCounting ? "Stop" : null} // Only show when counting
  onPress={() => setIsCounting(false)} 
  style={{ backgroundColor: 'red', paddingVertical: 18, marginTop: 10 }} 
/>
    </SafeAreaView>
  )
}
