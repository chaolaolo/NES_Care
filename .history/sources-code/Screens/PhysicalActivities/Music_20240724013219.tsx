...
const Music = () => {
    ...
    useEffect(() => {
        const updateTrackInfo = async () => {
            ...
            if (track) {
                ...
                setTrackDuration(await TrackPlayer.getDuration());
            }
        };
        ...
        const onPlaybackStateChange = async () => {
            if (await TrackPlayer.getState() === State.Playing) {
                ...
                setInterval(async () => {
                    setTrackPosition(await TrackPlayer.getPosition());
                }, 1000);
            }
        };
        ...
        return () => clearInterval(onPlaybackStateChange);
    }, [playerState]);
    ...
    // const handlePlay = async (item) => {
    //     try {
    //         // Setup player if not done already
    //         await setupPlayer();

    //         // Get the current track
    //         const currentTrack = await TrackPlayer.getCurrentTrack();
    //         if (currentTrack === item.key) {
    ...
    //             }
    //         } else {
    //             // If the track is different, reset and add the new track
    //             await TrackPlayer.reset();
    //             await TrackPlayer.add({
    //                 id: item.key,
    //                 url: item.link,
            ...
            {currentTrack ? (
                <View style={styles.playingContainer}>
                    ...
                    <View style={{ flex: 1, marginHorizontal: 16 }}>
                        ...
                        <View style={{ width: 210, marginLeft: 40, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                            <Image style={{ width: 20, height: 20 }} source={require('../../image/ic_previous_music.png')} />
                            <Image style={{ width: 40, height: 40 }} source={playerState === State.Playing ? require('../../image/ic_stop_music.png') : require('../../image/ic_playing_music.png')} />
                            <View style={{ flexDirection: 'row' }}>
                                <Image style={{ width: 20, height: 20 }} source={require('../../image/ic_next_music.png')} />
                                <Image style={{ width: 20, height: 20, marginLeft: 20 }} source={require('../../image/ic_play_again.png')} />
                            </View>
