...
// Placeholder function for step counting
...
const Move = () => {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <BlockComponent style={{ marginVertical: 10, paddingVertical: 20 }}>
        <Text style={styles.txtTitle}>Distance</Text>
        <Text style={styles.txtContent}>{(steps * 0.0008).toFixed(2)} km</Text> {/* Assuming 1 step = 0.8 meters */}
      </BlockComponent>
      <BlockComponent style={{ marginVertical: 5, paddingVertical: 20 }}>
        <Text style={styles.txtTitle}>Moving Time</Text>
        <Text style={styles.txtContent}>{`${Math.floor((steps / 120))}:${(steps % 120).toString().padStart(2, '0')}`}</Text> {/* Assuming 120 steps per minute */}
      </BlockComponent>
