...
const Move = () => {
  ...
  useEffect(() => {
    const startPedometer = async () => {
      try {
        const pedometer = await Pedometer.startPedometerUpdates()
        pedometer.subscribe(({ numberOfSteps }) => {
          setSteps(numberOfSteps)
          // Cập nhật distance và movingTime theo logic của bạn
        })
      } catch (error) {
        console.error(error)
      }
