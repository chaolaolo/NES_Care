...
const Music = () => {
    ...
    const handlePlay = async (item) => {
        try {
            ...
            if (currentTrack) {
                // Nếu có track đang phát
                const track = await TrackPlayer.getTrack(currentTrack);

                if (track.id === item.key) {
                    // Nếu track hiện tại là track người dùng đang chọn, dừng hoặc phát
                    const state = await TrackPlayer.getState();

                    if (state == State.Playing) {
                        TrackPlayer.pause(); // Dừng phát nhạc
                    } else {
                        TrackPlayer.play(); // Tiếp tục phát nhạc
                    }
                } else {
                    // Nếu track hiện tại khác với track người dùng chọn, đổi track và phát
                    await TrackPlayer.reset();
                    await TrackPlayer.add({
                        id: item.key,
                        url: item.link,
                ...
                // Nếu không có track nào đang phát, thêm và phát track mới
                await TrackPlayer.reset();
                await TrackPlayer.add({
                    id: item.key,
                    url: item.link,
