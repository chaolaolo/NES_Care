...
import { Pedometer } from '@gregfrench/react-native-pedometer';
  ...
  const handleButtonPress = () => {
    ...
    const watchId = Geolocation.watchPosition(
      position => {
        ...
        // Update distance calculation logic based on position.coords
      },
      error => {
        console.error(error);
      },
    ...
    Pedometer.startPedometerUpdatesFromDate(new Date().getTime(), pedometerData => {
      setSteps(pedometerData.numberOfSteps);
      setDistance(pedometerData.distance); // assuming distance is in meters
    });

    const intervalId = setInterval(() => {
      const now = new Date().getTime();
      setMovingTime(Math.floor((now - startTime) / 1000));
    }, 1000);
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <BlockComponent style={{ marginVertical: 5, paddingVertical: 20 }}>
        <Text style={styles.txtTitle}>Moving Time</Text>
        <Text style={styles.txtContent}>
          {Math.floor(movingTime / 60)}:{movingTime % 60}
        </Text>
      </BlockComponent>
      <Text style={{
        color: '#432C81',
        ...
        Now let's start your exercising,
        Click “Start” to start' your journey with NES Care!
      </Text>
      <ButtonComponent
        onPress={handleButtonPress}
        title={"Start"}
