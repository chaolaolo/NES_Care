...
const MoveComponent = memo(({ steps, distance, movingTime, calories, isCounting, startCounting, stopCounting, resetCounting }) => {
  ...
  const handleRefresh = async () => {
    setRefreshing(true);
    await fetchLatestTarget(); // Hoặc bất kỳ hàm nào bạn muốn gọi lại
    setRefreshing(false);
  };
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={{ backgroundColor: '#E4F9F3', padding: 10, margin: 10, marginTop: 20 }}>
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
            <Text style={{ fontSize: 16 }}>Mục tiêu thời gian:</Text>
            <Text style={{ fontSize: 16, fontWeight: 'bold', color: '#7B6BA8' }}>{movingTime}/{latestTarget.moveTimeTarget || 'chưa đặt mục tiêu'}</Text>
          </View>
        </View>
      ...
      >
        <TextInputComponent
          label="Mục tiêu số bước"
          value={stepsTarget}
          onChangeText={setStepsTarget}
          placeholder="Nhập số bước"
          error={errStepsTarget}
          keyboardType="numeric"
        />
        <TextInputComponent
          label="Mục tiêu quãng đường"
          value={distanceTarget}
          onChangeText={setDistanceTarget}
          placeholder="Nhập quãng đường"
          error={errDistanceTarget}
          keyboardType="numeric"
        />
        <TextInputComponent
          label="Mục tiêu thời gian"
          value={moveTimeTarget}
          onChangeText={setMoveTimeTarget}
          placeholder="Nhập thời gian"
          error={errMoveTimeTarget}
          keyboardType="numeric"
        />
      </ModalComponent>
      <ModalComponent
        modalVisible={showStopModal}
        onClose={() => setShowStopModal(false)}
        title="Dừng đếm bước"
        onPress={handleStopMove}
      >
        <Text>Bạn có chắc chắn muốn dừng đếm bước không?</Text>
      </ModalComponent>
      <ModalComponent
        ...
        title="Chúc mừng"
      >
        <Text>Bạn đã hoàn thành mục tiêu!</Text>
      </ModalComponent>
    </View>
  );
});
