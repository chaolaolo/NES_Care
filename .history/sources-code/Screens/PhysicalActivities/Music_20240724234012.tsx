...
const Music = () => {
        ...
        const updateTrackInfo = async () => {
            ...
            if (track) {
                ...
                if (trackDetails) {
                    ...
                    setCurrentTrack(trackDetails);
                    setMusicArtist(trackDetails.id);
                }
    ...
    //         //     console.error("No track found or item is null");
    //         //     return;
    //         // }
    ...
    const handlePlay = async (item) => {
        try {
            ...
            if (currentTrack === null || currentTrack !== item.key) {
                // Kiểm tra xem bài nhạc hiện tại có phải là bài nhạc mới không
                const isTrackInQueue = queue.some(track => track.id === item.key);
                
                if (!isTrackInQueue) {
                    // Nếu bài nhạc mới chưa có trong hàng đợi, thêm vào hàng đợi và phát
                    await TrackPlayer.reset();
                    await TrackPlayer.add({
                        id: item.key,
                        url: item.link,
                    ...
                    // Nếu bài nhạc mới đã có trong hàng đợi, chỉ cần phát bài đó
                    await TrackPlayer.play();
                    setIsPlaying(true);
                }
