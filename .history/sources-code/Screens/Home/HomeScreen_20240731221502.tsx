...
const HomeScreen = () => {
    ...
    const [moveTarget, setMoveTarget] = useState({ steps: 0, distance: 0 });
    ...
    useEffect(() => {
        const fetchLatestMove = async () => {
            try {
                const moveQuerySnapshot = await firestore()
                    .collection('Moves')
                ...
                if (!moveQuerySnapshot.empty) {
                    const latestMoveData = moveQuerySnapshot.docs[0].data();
                    setLatestMove({
                        steps: latestMoveData.steps,
                        distance: latestMoveData.distance,
                    });
                    ...
                    if (!targetQuerySnapshot.empty) {
                        const moveTargetData = targetQuerySnapshot.docs[0].data();
                        setMoveTarget({
                            steps: moveTargetData.steps,
                            distance: moveTargetData.distance,
                        });
                    }
                }
            } catch (error) {
                console.log('Error fetching move and target data: ', error);
            }
        };
            ...
            <ScrollView style={{ marginBottom: 60 }}>
                <View
                    ...
                    <BlockComponent
                        ...
                        <Text style={styles.contentItem}>Số bước: {latestMove.steps}/{moveTarget.steps}</Text>
                        <Text style={styles.contentItem}>Quãng đường: {latestMove.distance}/{moveTarget.distance} (m)</Text>
                    </BlockComponent>
                    <BlockComponent
