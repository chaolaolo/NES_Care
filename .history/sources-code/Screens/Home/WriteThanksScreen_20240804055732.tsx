...
const WriteThanksScreen = () => {
    ...
    const [privacyValue, setPrivacyValue] = useState(null);
    ...
    const handleUpdate = () => {
        if (selectedItem && grateful) {
            firestore()
                .collection('Thanks')
                .doc(selectedItem.id)
                .update({ content: grateful, privacy: privacyValue, })
                .then(() => {
    ...
    const handleupdateTarget = () => {
        if (targetValue) {
            firestore()
                ...
                .add({
                    target: targetValue,
                    categoryTarget: 'grateful',
                    timestamp: new Date(),
                    uid: user.uid
                })
                .then(() => {
                    Alert.alert("Thành công", "Bạn đã đặt mục tiêu thành công.")
                    setShowTarget(false);
                    setTargetValue(null);
                })
                .catch(error => {
                    Alert.alert("Thất bại", "Bạn đã đặt mục tiêu thất bại.")
                    console.error("Error adding document: ", error);
                });
        }
    };
