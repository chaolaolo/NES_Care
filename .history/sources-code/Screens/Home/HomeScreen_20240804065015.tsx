...
const HomeScreen = () => {
    ...
    //                 const targetData = targetsSnapshot.docs[0].data();
    //                 setGratefulTarget(targetData.target);  
    //                 // Tính số ngày duy trì
    //                 const startDate = new Date(targetData.timestamp.toDate()); // Giả sử có trường startDate
    //                 const today = new Date();
        ...
        const unsubscribe = firestore()
            ...
            .onSnapshot(querySnapshot => {
                if (!querySnapshot.empty) {
                    ...
                    const startDate = new Date(targetData.timestamp.toDate());
                    const today = new Date();
                    const dayDifference = Math.floor((today - startDate) / (1000 * 60 * 60 * 24));
                    setGratefulDays(dayDifference);
                } else {
            ...
            <ScrollView style={{ marginBottom: 60 }}>
                <View
                    ...
                    <BlockComponent
                        ...
                        <Text style={styles.contentItem}>Mục tiêu: {gratefulTarget}</Text>
