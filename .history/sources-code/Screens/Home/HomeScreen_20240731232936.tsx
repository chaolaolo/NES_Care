...
import { Button, Image, SafeAreaView, ScrollView, StyleSheet, Text, TouchableOpacity, View, RefreshControl } from 'react-native';
...
const HomeScreen = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            const startOfDay = new Date(today.setHours(0, 0, 0, 0));
            const endOfDay = new Date(today.setHours(23, 59, 59, 999));
            const gratefulSnapshot = await firestore()
                .collection('Thanks')
                .where('uid', '==', user.uid)
                .where('date', '>=', startOfDay)
                .where('date', '<=', endOfDay)
            ...
            if (!gratefulSnapshot.empty) {
                setGratefulCount(gratefulSnapshot.size);
                setGratefulDate('Hôm nay');
            } else {
                setGratefulCount(0);
            }
